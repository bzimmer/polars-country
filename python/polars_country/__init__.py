...
Exposes a single ``code`` expression that plugs into any Polars query.
...
# Narrower than polars.type_aliases.IntoExpr — only the two forms this plugin accepts.
CoordInput = str | pl.Expr
...
def code(coords: CoordInput | list[CoordInput]) -> pl.Expr:
    ...
    coords:
        ...
        - A column name or ``pl.Expr`` whose values are ``[lat, lng]`` lists
          or fixed-size arrays (``pl.List`` or ``pl.Array``).
    ...
    if isinstance(coords, list):
        ...
        # Cast to List(Float64) so both pl.List and pl.Array columns are handled
        # uniformly, then unpack both fields in a single list traversal via
        # to_struct before passing to the plugin.
        latlng = _to_expr(coords).cast(pl.List(pl.Float64))
        packed = latlng.list.to_struct(fields=["lat", "lng"])
        lat_expr = packed.struct.field("lat")
        lng_expr = packed.struct.field("lng")
